...
class DataBase:
    ...
    async def get_balance(self):
        result = await self.execute_query(
            '''DELETE FROM users
            WHERE id IN (SELECT MAX(id) FROM users)
            ''')  # Обратите
        # внимание на запятую, чтобы создать кортеж
        return result

    async def get_users(self):
        result = await self.execute_query('''SELECT SUM(balance) FROM users''')
        return result[0][0]  # Обратите внимание на запятую, чтобы создать кортеж

    async def get_osall(self):
